...
```xml
<description>Esta skill analisa as mudanças no repositório, cruza com contexto disponível (conversa, PR-PLAN, docs/plan), propõe uma divisão em commits atômicos e executa conforme a escolha do usuário — todos de uma vez ou um por vez.</description>
```
...
2. **`docs/review-pr/PR-PLAN-*.md`** — plano gerado a partir de comentários de PR
...
<commit-message>
	...
	<footer>(opcional: ex. BREAKING CHANGE: detalhes, referências a issues, ou Refs: docs/review-pr/PR-PLAN-N.md)</footer>
</commit-message>
```
...
<validation>
	...
	<footer>Use para breaking changes, referências a issues ou ao PR-PLAN quando aplicável.</footer>
</validation>
```
